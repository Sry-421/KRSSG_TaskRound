...
void reached()
{
	...
	{
		if(a.x!=end_point.x && a.y!=end_point.y)
			final_path.push(a);
	...
	////////////////////////////
	int count=1;
	int total_points=final_path.size();
	ofstream file;
	file.open("data.txt");

	file<<(int)(final_path.size()/5.0)+2<<endl;

	file <<	((start_point.x*10.0)/img.cols+1) <<endl;
	file << (10.0-((start_point.y*10.0)/img.rows)+1) <<endl;
	cout <<	((start_point.x*10.0)/img.cols+1) <<" ";
	cout << (10.0-((start_point.y*10.0)/img.rows)+1) <<endl;	
	
	while(!final_path.empty())
	{
		node temp=final_path.top();	
		final_path.pop();
		float a=(temp.x*10.0)/(img.cols)+1;
		float b=10.0-((temp.y*10.0)/(img.rows))+1;	
		cout <<a<<" "<<b<<endl;
		if((count)%5==0)
		{
			file <<a<<endl;
	...
	file <<	((end_point.x*10.0)/img.cols+1) <<endl;
	file << (10.0-((end_point.y*10.0)/img.rows)+1) <<endl;

	cout <<	((end_point.x*10.0)/img.cols+1) <<" ";
	cout << (10.0-((end_point.y*10.0)/img.rows)+1) <<endl;		
